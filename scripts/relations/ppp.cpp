...
void getData()
{
    ...
    for (int i = 1; i < content.size(); i++)
    {
        ...
        for (int j = 0; j < content[i].size(); j++)
        {
            ...
            {
                if (content[i][j] == "OTHERS" || content[i][j] == "UNDEFINED")
                    m = 2;
                else if (content[i][j] == "MALE")
                    m = 1;
                else
                    m = 0;
            }
...
void create(string name, string f, string Fname, string age)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (peeps[i]->gender == 1)
            fout << "MALE";
        else if (peeps[i]->gender == 2)
            fout << "UNDEFINED";
        else
            fout << "FEMALE";
        fout << ",";
        if (peeps[i]->age == INT_MAX)
            cout << "UNDEFINED";
        else
            fout << peeps[i]->age;
        fout << ",";
        if (peeps[i]->father)
            fout << peeps[i]->father->name;
        else
            fout << 0;
        fout << ",";
        if (peeps[i]->mother)
            fout << peeps[i]->mother->name;
        else
            fout << 0;
        fout << ",";
        if (peeps[i]->spouse)
            fout << peeps[i]->spouse->name;
        else
            fout << 0;
        fout << ",";
        if (peeps[i]->FIL)
            fout << peeps[i]->FIL->name;
        else
            fout << 0;
        fout << ",";
        if (peeps[i]->MIL)
            fout << peeps[i]->MIL->name;
        else
            fout << 0;
        fout << ",";
...
void createJson(string name, string f, string Fname, string age)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        fout << "\"Father\""
             << ":"
             << "\"" << (!(peeps[i]->father) ? "_Not_defined_" : peeps[i]->father->name) << "\""
             << "," << endl;
        fout << "\"Mother\""
             << ":"
             << "\"" << (!(peeps[i]->mother) ? "_Not_defined_" : peeps[i]->mother->name) << "\""
             << "," << endl;
        fout << "\"Spouse\""
             << ":"
             << "\"" << (!(peeps[i]->spouse) ? "_Not_defined_" : peeps[i]->spouse->name) << "\""
             << "," << endl;
        fout << "\"FIL\""
             << ":"
             << "\"" << (!(peeps[i]->FIL) ? "_Not_defined_" : peeps[i]->FIL->name) << "\""
             << "," << endl;
        fout << "\"MIL\""
             << ":"
             << "\"" << (!(peeps[i]->MIL) ? "_Not_defined_" : peeps[i]->MIL->name) << "\""
             << "," << endl;
        fout << "\"Children\""
             << ":";
        int z = peeps[i]->children.size();
...
int32_t main(int32_t argc, char **argv)
{
    Radhe Krishna
            cout
        << argv[0] << " " << argv[1] << " " << argv[2] << " " << argv[3] << " " << stoi(argv[4]) << endl;
